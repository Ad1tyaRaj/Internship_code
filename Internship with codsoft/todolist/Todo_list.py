...
class My_TodoList:
    def __init__(self, root):
        ...
        self.add_button = tk.Button(root, text="Add Task", command=self.add_task,bg="#89ff74",font="Arial 10 bold")
        self.add_button.grid(row=0, column=2, padx=5, pady=10)

        self.remove_button = tk.Button(root, text="Remove Task", command=self.remove_task,bg='#ff884d',font="Arial 10 bold")
        self.remove_button.grid(row=1, column=2, padx=5, pady=5)

        self.clear_button = tk.Button(root, text="Clear All Tasks", command=self.clear_tasks,bg='gray',font='Comicsensms 14 bold')
        self.clear_button.grid(row=2, column=0, padx=5, pady=5)

        self.exit_button = tk.Button(root, text="Exit",command=self.exit_app,bg='#0cfffc',font='Comicsensms 14 bold')
...
def main():
    ...
    root.wm_iconbitmap("todoicon.ico")
